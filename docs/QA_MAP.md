...
| UI | Local web UI localhost binding | UI accidentally exposes a listener to LAN or wider network | `cmd/velesmist/ui_test.go` | `go test ./cmd/velesmist` | implemented |
| UI | UI scan path reuses scanner contract | Browser workflow drifts from CLI behavior or report schema | `cmd/velesmist/ui_test.go`, `cmd/velesmist/main.go` | `go test ./cmd/velesmist ./internal/contracts` | implemented |
| UI | UI error responses avoid leaking request URLs | Upstream/network failures expose Steam identifiers or URLs in browser-visible API errors | `cmd/velesmist/ui_test.go` | `go test ./cmd/velesmist` | implemented |
| UI | UI contract coverage gate | UI endpoint/HTML/API behavior loses test evidence after refactors | `cmd/velesmist/ui_test.go`, `Makefile` | `make coverage-ui` | implemented |
...
| Steam provider | Safe paginated inventory fetch | Large Dota 2 inventories are rejected by Steam when requested with an oversized `count` value | `internal/steam/client_test.go` | `go test ./internal/steam` | implemented |
| Steam provider | Private/unavailable inventory error clarity | Users cannot distinguish network success from private inventory denial | `internal/steam/client_test.go` | `go test ./internal/steam` | implemented |
...
| QA evidence | Focused UI/CLI coverage threshold | A UI surface is shipped without enough contract-level automated checks | `Makefile`, `cmd/velesmist/ui_test.go` | `make coverage-ui` | implemented |
...
`make verify` is the normal deterministic local gate. It includes the focused `cmd/velesmist` UI/CLI coverage gate, but does not enforce a repository-wide coverage percentage. Network/security checks such as `make vuln` stay separate because they may need module or vulnerability database access.
